  ...
  return (
    <div className=''>
      ...
      <div className='p-3'>
        ...
        { isSuccess && data.data.categories.map((category:any)=>(
            <div key={`categoriesList${category.id}`} className="">
              <div className='flex gap-2 border-b-3 border-gray-300 min-h-[90px] py-2 items-center'>
                  ...
                  <div className='w-full font-bold flex gap-2'>
                    <button onClick={()=>router.push(`/categories/edit/${category.id}`)}  className='px-3 py-2 rounded-xl bg-gray-400 text-white'>edit</button>
                  </div>
              </div>
            </div>
        ))}
